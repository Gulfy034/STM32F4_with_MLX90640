...
int MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress,uint16_t nMemAddressRead, uint16_t *data)
{
	uint16_t *p;
	p=data;
  HAL_I2C_Mem_Read(&hi2c1,slaveAddr , startAddress, I2C_MEMADD_SIZE_16BIT,(uint8_t *)i2cData ,nMemAddressRead*2 , 1000);
	
    for(int cnt=0; cnt < nMemAddressRead; cnt++)
    {
        *p++ = (int)i2cData[cnt*2]*256 + (int)i2cData[cnt*2+1];
    } 
    return 0;
}
...
int MLX90640_DumpEE(uint8_t slaveAddr, uint16_t *eeData)
{
	...
	MLX90640_I2CRead(slaveAddr, (uint16_t)0x2400,832, eeData);
...
int MLX90640_I2CWrite(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
    ...
    cmd[0] = data >> 8;
    cmd[1] = data & 0x00FF;
 HAL_I2C_Mem_Write(&hi2c1, slaveAddr, (uint16_t)writeAddress, I2C_MEMADD_SIZE_16BIT, cmd,2, 1000);
return 0;
}
